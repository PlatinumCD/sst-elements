...
public:
    ...
    {
        uint32_t logicalCoreId = process->getCore() * os->getNumHwThreads() + process->getHwThread();
        m_output->verbose(CALL_INFO, 16, 0, "[syscall-getcpu] logicalcpu=%d\n", logicalCoreId);
        setReturnSuccess(logicalCoreId);
    }
};
