#ifndef _EMULATEDCOMPUTEARRAY_H
#define _EMULATEDCOMPUTEARRAY_H
...
template<typename T>
class EmulatedComputeArray : public ComputeArray {
public:
    SST_ELI_REGISTER_SUBCOMPONENT_DERIVED_API(
            EmulatedComputeArray<T>, 
            SST::Golem::ComputeArray,
            TimeConverter*,
            Event::HandlerBase*
    )

    EmulatedComputeArray(ComponentId_t id, Params& params,
                         TimeConverter* tc,
                         Event::HandlerBase* handler)
        : ComputeArray(id, params, tc, handler) {
        // Configure selfLink
        selfLink = configureSelfLink("Self", tc, new Event::Handler<EmulatedComputeArray>(this, &EmulatedComputeArray::handleSelfEvent));
