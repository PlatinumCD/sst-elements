...
# Force exactly 3 cores (numCpus=3), each with 1 thread
numCpus = 3
numThreads = 1
...
# Process list: run openmp_example with OMP_NUM_THREADS = 3 * 1 = 3
processList = (
    (
        1,
        {
...
# Combine roccParams + arrayParams
...
class CPU_Builder:
    ...
    def build(self, prefix, nodeId, cpuId):
        ...
        # ------------------------------
        # Only add RoCC on Cores > 0
        # ------------------------------
        if cpuId > 0:
            cpu_rocc = cpu.setSubComponent("rocc0", "golem.RoCCAnalogInt")
            ...
            roccDcacheIf = None  # No RoCC for CPU 0
        ...
        # Return OS link, L2->mem link, dtlb link, itlb link
