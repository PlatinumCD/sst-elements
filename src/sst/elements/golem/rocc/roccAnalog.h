...
public:
  ...
  RoCCAnalog(ComponentId_t id, Params &params)
    ...
    if (!array) {
      output->fatal(CALL_INFO, -1,
                    "Unable to load array model subcomponent.\n");
    }
  }

  virtual ~RoCCAnalog() {
    for (auto roccCmd_q_itr = roccCmd_q.begin();
         roccCmd_q_itr != roccCmd_q.end();) {
  ...
  void storeVector() {
    ...
    for (size_t i = 0; i < static_cast<size_t>(arrayOutputSize); i++) {
      if constexpr (std::is_same<T, float>::value || std::is_same<T, double>::value) {
        ...
        output->output("%lld ", static_cast<long long>(outputVector[i]));
      }
    }
    output->output("\n\n");
    ...
    uint32_t request_size = static_cast<uint32_t>(std::min(
        cache_line_size - addr_offset, vector_total_size - write_offset));
  ...
  void moveVector() {
    ...
    for (int i = 0; i < arrayInputSize; i++) {
      if constexpr (std::is_same<T, float>::value || std::is_same<T, double>::value) {
        output->output("%f ", static_cast<double>(inputVector[i]));
      } else {
        output->output("%ld ", static_cast<long>(inputVector[i]));
      }
    }
    output->output("\n");
  ...
  public:
    ...
    virtual void handle(StandardMem::ReadResp *ev) {
      ...
      {
        rocc->output->verbose(CALL_INFO, 2, 0,
    ...
    virtual void handle(StandardMem::WriteResp *ev) {
      ...
      if (ev->getFail()) {
        ...
        if (rocc->write_offset < rocc->vector_total_size) {
          ...
          uint64_t cache_line_size = rocc->memInterface->getLineSize();
          uint32_t request_size = static_cast<uint32_t>(std::min(
              cache_line_size, rocc->vector_total_size - rocc->write_offset));
